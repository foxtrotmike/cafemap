...
class LLC(LocalEncoder):
    def __init__(self,X, K = None, k = None, gamma = 1e-3, **kwargs):
        """
    ...
    #                must be used when data is sparse.")
                if 'rand' in kwargs and kwargs['rand']:
                    print('Using Random2')                 
                    ridx = np.random.choice(X.shape[0],C)  #random indices
                    C = X[ridx,:]
                else:
        ...
        c,_ = C.shape
        print("Number of Clusters",C.shape)
...
#        import pdb; pdb.set_trace()
        ...
        if issparse(X):
            blocks = int(min(max((1,(X.shape[0]/C.shape[0])/2)),X.shape[0]))
        else:
            blocks = 1        
            Xd = X

        for j in range(blocks):
           begin = int(j*N/blocks )
           if issparse(X):
               Xd = X[begin:int((j+1)*N/blocks),:].toarray()    
           for i in range(Xd.shape[0]):
               ...
               #import pdb; pdb.set_trace()
               Z = Z + II*gamma*np.trace(Z);            # regularlization (K>D)
...
#        )) 
    ...
    llc = LLC(X, K = C, k = 10, gamma = 0.002, sample = samples) #, reconstruct = True
    G = llc.encode(X)
